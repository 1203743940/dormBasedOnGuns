...
public class ConstantFactory implements IConstantFactory {
    ...
    private RoomMapper roomMapper = SpringContextHolder.getBean(RoomMapper.class);
    ...
    @Override
    public String getRoomRoomId(Long roomId) {
        return null;
    }

    @Override
    @Cacheable(value = Cache.CONSTANT, key = "'" + CacheKey.DEPT_NAME + "'+#buildingId")
    public String getBuildingName(Long buildingId) {
        if (buildingId == null) {
            return "";
        } else if (buildingId == 0L) {
            return "顶级";
        } else {
            Building building = buildingMapper.selectById(buildingId);
            if (ToolUtil.isNotEmpty(building) && ToolUtil.isNotEmpty(building.getFullName())) {
    ...
    @Override
    @Cacheable(value = Cache.CONSTANT, key = "'" + CacheKey.DEPT_NAME + "'+#roomId")
    public String getRoomName(Long roomId) {
        if (roomId == null) {
            ...
            if (ToolUtil.isNotEmpty(room) && ToolUtil.isNotEmpty(room.getPids())) {
                return room.getPids();
            }
            return "";
        }
